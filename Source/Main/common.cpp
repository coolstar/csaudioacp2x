...
private:
    ...
    PCCsAudioAcp2xHW        m_pHW;                  // HW object
    ...
    (
        ...
        _In_reads_opt_(cPropertyCount) const CSAUDIOACP2X_DEVPROPERTY* pProperties,
...
NTSTATUS CsAudioAcp2xIoSetDeviceInterfacePropertyDataMultiple
(
    _In_ PUNICODE_STRING                                        SymbolicLinkName,
    _In_ ULONG                                                  cPropertyCount,
    _In_reads_opt_(cPropertyCount) const CSAUDIOACP2X_DEVPROPERTY        *pProperties
...
--*/
{
    ...
    // Initialize HW.
    // 
    m_pHW = new (NonPagedPool, CSAUDIOACP2X_POOLTAG)  CCsAudioAcp2xHW(ResourceList);
    if (!m_pHW)
    {
...
--*/
{
    ...
    //
    ntStatus = CsAudioAcp2xIoSetDeviceInterfacePropertyDataMultiple(AudioSymbolicLinkName, cPropertyCount, pProperties);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: CsAudioAcp2xIoSetDeviceInterfacePropertyDataMultiple(...): failed, 0x%x", ntStatus)),
        Done);
...
)
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology || NULL == unknownMiniTopo)
    {
        ...
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with CSAUDIOACP2X.<TopoName>.szPname in the inf's [Strings] section
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave)
    {
        ...
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with CSAUDIOACP2X.<WaveName>.szPname in the inf's [Strings] section
